...
(defn- do-upload-and-callbak
  [{qiniu-key    :qiniu_key qiniu-secret :qiniu_secret qiniu-bucket :qiniu_bucket
    content-type :content_type upload-type :upload_type {file-size :size file :tempfile} :file filepath :filepath
    return-type  :return_type webhook-url :web_hook_url :as params} file-key]
  (let [_ (do-upload params file-key)]
    (client/get (str webhook-url "?success=true&file-size=-1&store_key=" file-key))
    (logging/info "hi, agent done")))                       ;; TODO: change file-size
...
(defn upload-handler
  ...
  (do
    ...
    (case return-type
      "direct" (json/write-str {:success true :file-size file-size :store_key (do-upload params)})
      ...
      "error")
    ;; 暂时只直接上传
    ;(let [file-key (do-upload params)]
    ;  (json/write-str {:success true :file_size file-size :store_key file-key}))
    ))

(defroutes api-routes
           (GET "/api/v1/hi" [] (hi-api))
